...
class ModelTraining:
    def __init__(self, data_path):
        ...
        self.experiment = comet_ml.Experiment(
            api_key="ZKya8AZhOR4pW4sNhDjEHkdXQ",
    ...
    def train_model(self):
        try:
            ...
            def lrfn(epoch):
                if epoch < ranup_epochs:
                    return (max_lr-start_lr)/ranup_epochs * epoch + start_lr
                elif epoch < ranup_epochs + sustain_epochs:
                    return max_lr
                else:
                    return (max_lr-min_lr) * exp_decay ** (epoch - ranup_epochs - sustain_epochs) + min_lr
    ...
    def extract_weights(self, name, model):
        try:    
            ...
            logger.info(f"Extracted weights for {name} layer")
            return weights
        except Exception as e:
